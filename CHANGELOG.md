...
### Added
- **排行榜HTML图片展示**: 群排行榜现在以精美的图片形式展示
  - 新增领奖台设计，前三名有金色/银色/铜色特效
  - 显示打卡次数、参与人数统计
  - 保留文本降级方案，渲染失败时自动切换
- **年度热力图功能**: 类似GitHub贡献图的风格，展示一年内打卡记录
  - 命令 `/heatmap` 或 `/热力图` - 查看今年打卡热力图
  - 支持查看指定年份: `/heatmap 2025`
  - 黄色系配色，颜色越深表示当天打卡次数越多
  - 显示统计信息: 打卡天数、总次数、单日最多、日均次数
  - 支持月标签和星期标签

### Changed
- 排行榜命令现在使用HTML渲染生成图片
  - `/leaderboard`, `/yesterday_rank`, `/monthly_rank` 均改为图片输出
- **日历标题优化**: 非本月日历现在显示完整年月标题
  - 本月日历标题: `X月`
  - 非本月日历标题: `XX年X月`
  - 通过 `is_current_month` 变量控制模板渲染

### Fixed
- **批量报告渲染截断**: 修复5人以上批量打卡时图片渲染不完整的问题
  - 将 `full_page: False` 改为 `full_page: True`，移除固定高度限制
  - 浏览器自动检测完整页面高度，确保无论多少人都能完整渲染

## [1.1.1] - 2026-05-03

### Added
- **群排行榜功能**: 支持查看群内打卡排行榜
  - `/leaderboard` 或 `/排行榜` - 查看今日群打卡排行榜
  - `/yesterday_rank` 或 `/昨日榜` - 查看昨日群打卡排行榜
  - `/monthly_rank` 或 `/月榜` - 查看本月群打卡排行榜
  - 排行榜显示前10名用户，附带奖牌和统计信息
